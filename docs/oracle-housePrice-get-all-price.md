...
预言机数据来源于百万级持续更新的数据库，由预言机喂价节点通过分布式数据模式将处理过的数据上报到

智能合约链接预言机可使用housePriceInterface接口，调用getAll()方法，一次性获取到近12个月的房价实际数据。
...
contract PriceConsumer {
    ...
    function getLatestPrice() public view returns (uint[12] memory prices) {
        uint[12] prices = priceOracle.getAll(1); // 假设希望得到New York的房价信息，city参数填1
        return;
    }
}
```
